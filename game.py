...
class Q20Game:
    def __init__(
        self,
        item: str,
        answerer_model_name: str = "gpt2",  # Specify default Hugging Face model
        guesser_model_name: str = "gpt2",   # Specify default Hugging Face model
        num_turns: int = 20,
        ...
        self.guesser_model_name = guesser_model_name
        ...
        self.guesser_tokenizer = AutoTokenizer.from_pretrained(guesser_model_name)
        self.guesser_model = AutoModelForCausalLM.from_pretrained(guesser_model_name)
        self.answerer_tokenizer = AutoTokenizer.from_pretrained(answerer_model_name)
        self.answerer_model = AutoModelForCausalLM.from_pretrained(answerer_model_name)

        self.guesser_messages = []

    def guesser(self, messages):
        prompt = self.dialog_history() + "ASSISTANT:"
        input_ids = torch.tensor([self.guesser_tokenizer.encode(prompt, add_special_tokens=True)])
        input_ids = input_ids.to(self.guesser_model.device)
        ...
        with torch.no_grad():
            ...
            gen_str = self.guesser_tokenizer.decode(gen[0][input_ids[0].shape[0]:]).strip()

        return {"role": "assistant", "content": gen_str}

    def answerer(self, question):
        prompt = f"For the entity {self.item}, {question} (Yes/No/Maybe)"
        input_ids = self.answerer_tokenizer.encode(prompt, return_tensors="pt").to(self.answerer_model.device)

        with torch.no_grad():
            output = self.answerer_model.generate(input_ids, max_length=1024, temperature=0.2)
            response = self.answerer_tokenizer.decode(output[0], skip_special_tokens=True)

        return {"role": "user", "content": response.strip()}

    def dialog_history(self):
        history = ""
        for message in self.guesser_messages:
